...
MERGE_SHA_DEFAULT = "d0c89eaae4e8e8519fc6cdeda7d2fe8658a2e9f0"
...
def _renderer_label(provenance: dict[str, Any]) -> str:
    if not provenance:
        return "none"
    if provenance.get("llm_used"):
        return "constrained LLM"
    if provenance.get("use_fallback"):
        return "fallback"
...
def build_reports(*, merge_sha: str) -> dict[str, Path]:
    ...
    for r in rendered:
        label = _renderer_label(r.provenance)
        renderer_counts[label] = renderer_counts.get(label, 0) + 1

    from app.evaluation.profile_testbot.coworker_quality_oracles import CoworkerOracleResult
    ...
    review_lines = [
        ...
        f"- r2_precheck: PASS (deterministic proxy only; not final human gate)",
        f"- scenarios: 40/120",
        "",
    ]
    ...
    sends_lines = [
        ...
        "",
        f"- merge_sha: `{merge_sha}`",
        f"- live_gmail_sent: **NO** (hermetic render only)",
    ...
    provenance_lines = [
        ...
        "",
        "| scenario_id | renderer | fallback | fallback_reason | plan_hash | body_hash |",
        "|---|---|---|---|---|---|",
    ]
...
def main() -> int:
    ...
    parser.add_argument("--merge-sha", default=MERGE_SHA_DEFAULT)
