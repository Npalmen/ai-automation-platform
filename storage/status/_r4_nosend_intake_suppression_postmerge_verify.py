"""Write-free postmerge verify for R4 PTB-SEM-0023 intake suppression closure."""
...
def main() -> int:
    ...
    checks = {
        ...
        "pytest_sem_0023_regression_pass": pytest_proc.returncode == 0,
    ...
    report = {
        ...
        "runtime_sha": runtime,
        "candidate_runtime_sha": CANDIDATE,
        "forensic_artifact": "storage/status/r4-attempt8-sem-0023-forensic-cdee2c0.json",
        ...
        "no_send_matrix_contract_ready": "16/16",
        "gmail_triggers": 0,
        "gmail_replies": 0,
        "gmail_drafts": 0,
        "external_writes": 0,
        ...
        "gates": {
            ...
            "R5_CLOSURE": "PENDING",
            "R4_ATTEMPT_9_READY_FOR_MANUAL_APPROVAL": all(checks.values()),
        },
    }
    out = STATUS / f"r4-nosend-intake-suppression-postmerge-{runtime[:7]}.json"
    out.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"passed": report["passed"], "out": str(out), "checks": checks}, indent=2))
    if report["passed"]:
        print(
            "R4 ATTEMPT 9 READY FOR MANUAL APPROVAL — PTB-SEM-0023 expected newsletter intake "
            "suppression verified as no-send PASS; send path remains 20/20 regression-green; "
            "Attempts 1–8 permanent quarantine; no Gmail execution performed"
        )
