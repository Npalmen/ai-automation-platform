"""Write-free postmerge verify for R4 PTB-DCQ-0007 intake routing closure."""
...
def main() -> int:
    ...
    sem_0023 = resolve_r4_no_send_intake_suppression(
    ...
    checks = {
        ...
        "pytest_intake_regression_pass": pytest_proc.returncode == 0,
        ...
        "ptb_dcq_0007_attempt9_regression_pass": bool(
        ...
        "ptb_sem_0023_contract_ready": sem_0023.eligible is True,
    ...
    report = {
        ...
        "runtime_sha": runtime,
        "candidate_runtime_sha": CANDIDATE,
        "forensic_artifact": "storage/status/r4-attempt9-0007-intake-forensic-2b955a1.json",
        ...
        "ptb_sem_0023_expected_intake_suppression": "PASS" if sem_0023.eligible else "FAIL",
        "gmail_triggers": 0,
        "gmail_replies": 0,
        "gmail_drafts": 0,
        "external_writes": 0,
        ...
        "gates": {
            ...
            "R5_CLOSURE": "PENDING",
            "R4_ATTEMPT_10_READY_FOR_MANUAL_APPROVAL": all(checks.values()),
        },
    }
    out = STATUS / f"r4-intake-routing-postmerge-{runtime[:7]}.json"
    out.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"passed": report["passed"], "out": str(out), "checks": checks}, indent=2))
    if report["passed"]:
        print(
            "R4 ATTEMPT 10 READY FOR MANUAL APPROVAL — PTB-DCQ-0007 legitimate solar lead continuation "
            "verified through production intake; full R4 live-trigger intake compatibility 35/35 PASS; "
            "send/no-send/0088 regressions green; Attempts 1–9 permanent quarantine; no Gmail execution performed"
        )
