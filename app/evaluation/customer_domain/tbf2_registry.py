"""TBF2 campaign manifest registry."""
...
MANIFEST_PATH = Path(__file__).resolve().parent / "resources" / "tbf2_manifest.yaml"
EXPECTED_SCENARIO_IDS = tuple(f"TBF2-{index:02d}" for index in range(1, 11))
...
def load_manifest() -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("TBF2 manifest must be a mapping")
    return data
...
def manifest_scenarios() -> list[dict[str, Any]]:
    ...
    if not isinstance(scenarios, list):
        raise ValueError("TBF2 manifest scenarios must be a list")
    return scenarios
...
def validate_manifest() -> list[str]:
    ...
    if ids != list(EXPECTED_SCENARIO_IDS):
        failures.append(
            f"TBF2 manifest scenario IDs must be {list(EXPECTED_SCENARIO_IDS)}, got {ids}"
        )
    if len(set(ids)) != len(ids):
        failures.append("TBF2 manifest scenario IDs must be unique")
    for entry in scenarios:
        scenario_id = entry.get("scenario_id")
        module_name = entry.get("module")
        if not scenario_id or not module_name:
            failures.append(f"TBF2 manifest entry missing scenario_id/module: {entry}")
            continue
...
def load_tbf2_runners() -> dict[str, Callable[[EvalContext], ScenarioRunResult]]:
