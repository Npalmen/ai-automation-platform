"""Safety guards for full-function matrix evaluation."""
...
EVAL_TENANT_PREFIX = "eval_ff_"
REQUIRED_DB_NAME_FRAGMENT = "full_function_eval"
...
def assert_eval_environment() -> None:
    if not is_env_allowed():
        ...
        raise EvalGuardError(f"ENV '{env}' is not allowlisted for full-function evaluation.")


def assert_eval_database_url(database_url: str) -> str:
    if not database_url.strip():
        raise EvalGuardError("DATABASE_URL is required for full-function evaluation.")
    dialect = database_url.split(":", 1)[0].lower()
    if dialect not in {"postgresql", "postgres"}:
        raise EvalGuardError("Full-function evaluation requires PostgreSQL.")
