...
def _approval_title(job: Job, detected_job_type: str) -> str:
    return f"Approval required for {detected_job_type} ({job.job_id})"
...
def _approval_summary(
    *,
    ...
    confidence = (
        previous_payload.get("confidence")
        or policy_payload.get("invoice_confidence")
        or policy_payload.get("decisioning_confidence")
        or policy_payload.get("classification_confidence")
    )

    parts = [
        f"Job {job.job_id} requires approval.",
        f"Type: {detected_job_type}.",
    ]
    ...
    if reasons:
        parts.append(f"Reasons: {', '.join(sorted(set(reasons)))}.")
...
def build_approval_request(job: Job) -> dict[str, Any]:
    ...
    previous_payload = (job.result or {}).get("payload") or {}
    detected_job_type = policy_payload.get("detected_job_type", "unknown")
    ...
    return {
        ...
        "requested_by": "system",
        "title": _approval_title(job, detected_job_type),
