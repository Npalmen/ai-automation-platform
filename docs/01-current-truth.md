...
| Live Gmail transport testbot (2F.2) | `Implementation merged — live verification pending` | **Merged to `main` @ `4d343f8` (PR #11, 2026-07-22).** Two-process architecture (testbot send + app delivery/intake); S01 inbound-only with `fixture_ai`; hermetic safety contracts (telemetry `integration_type`, tenant allowlist on status mutation, Gmail pagination/truncation fail-closed, recipient header validation, cross-host writer-lock policy, workflow cleanup gate, DecisionRecord redaction); manual `live_gmail_transport` workflow (`workflow_dispatch` only). Security review F-01–F-07 closed; F-08 (OAuth seed DB guard) documented LOW/deferred. **No real Gmail send, no live LLM, no operator secrets configured.** Verification @ merge-head (Release Gate `29926558386`): hermetic 2F.2 tests 107 PASS (local); 2E smoke 10/10 PASS (`real_external_calls=0`); full backend 4002 PASS (0 failures); live-eval PG 5/5; migration 016 PG 1/1; telemetry idempotency PG 2/2; frontend PASS; Docker PASS. Gmail sends to date: 0. **Next:** operator configuration and first manual Live Gmail S01 verification. **Not completed.** 2F.3 not started. |
