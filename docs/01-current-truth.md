...
2026-08-08 (R5 profile-driven digital coworker reply quality closure — write-free; `PROFILE_DRIVEN_DIGITAL_COWORKER_REPLY_QUALITY_QUALIFIED=VALID`; registry provenance bound to qualifying R4 Attempt-12 runtime `4ad74d4ac19011d5edfb8ea160112f649052422d` + Release Gate `31220948265` (PR #183 merge of executor); R4 Attempt 12 sole PASS campaign `b4dcd6a8-9bda-4ce4-8b63-4e7a54176605` — 36/36, send 20/20, no-send 16/16, Gmail triggers 35 replies 20 drafts 0; R1 hermetic PASS; R2 human review PASS (`digital-coworker-r4-human-review-scored-b7fd95e.json`, sha256 `7dced592…`, 0 FAIL/0 PENDING); R3 live canary PASS (`5e9b1839…`); Attempts 1–11 permanent quarantine excluded; closure PR merge SHA documented separately as postmerge evidence only; `PROFILE_DRIVEN_AUTOMATIC_GMAIL_QUALIFIED` + `PROFILE_DRIVEN_TESTBOT_PASS` remain PENDING; automatic Gmail false; production activation false. Evidence: `storage/status/r5-closure-evidence-freeze-4ad74d4.json`, `storage/status/digital-coworker-r4-attempt12-pass-record-4ad74d4.json`.)

2026-08-07 (R4 live campaign attempt 12 PASS — executor `4ad74d4ac19011d5edfb8ea160112f649052422d` / candidate `b7fd95e…`; campaign `b4dcd6a8-9bda-4ce4-8b63-4e7a54176605`; fresh tenant seed + execute-time full JIT PASS; 36/36 PASS send 20/20 no-send 16/16; PTB-DCQ-0007 + PTB-DCQ-0088 full lifecycles PASS; PTB-SEM-0023 newsletter suppression PASS scenario-local delta 0; Attempts 1–11 excluded; `R4_LIVE_CAMPAIGN=PASS`; `R5_CLOSURE=PENDING` until registry closure. Evidence: `storage/status/digital-coworker-r4-attempt12-pass-record-4ad74d4.json`.)

2026-08-07 (R4 PTB-DCQ-0007 intake routing closure postmerge @ `6465938ac459459a09e6d64c6e6bd70ac7efdabe` — PR #182 squash-merged CI PASS; root cause: production `classify_email_type` scanned raw live-eval KROWOLF subject token where evaluation_run_id UUID `1e768ca6-…-b2b2-…` falsely matched partnership keyword `b2b`; semantic content classifies as `lead`; fix strips eval transport metadata before intake classification; new write-free `r4_live_intake_compatibility=35/35`; PTB-DCQ-0007 Attempt-9 UUID regression PASS; partnership/newsletter suppression preserved; `r4_live_gmail_scenario_eligibility=36/36`; exact registration 20/20+16/16; PTB-DCQ-0000→0002 PASS; PTB-DCQ-0088 materialization PASS; PTB-SEM-0023/0024 PASS; API=worker=`6465938`; registration probe PASS (0 Gmail writes); Attempts 1–9 permanent quarantine; no `--execute`; no Attempt-10 approval; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_ATTEMPT_10_READY_FOR_MANUAL_APPROVAL`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` remain PENDING; automatic Gmail false; production activation false. Evidence: `storage/status/r4-intake-routing-postmerge-6465938.json`, `storage/status/r4-attempt9-0007-intake-forensic-2b955a1.json`.)

2026-08-07 (R4 live campaign attempt 9 STOPPED — executor `2b955a1b9db6d38460dae881e4211cbe014fe775` / candidate `b7fd95e…`; campaign_id `8e0fa53a-868f-454f-b40f-2a41f94b2efe`; manual approval validated; preflight/JIT PASS; send 3/20 PASS (`PTB-DCQ-0000`, `0002`, `0005`); `PTB-DCQ-0007` FAIL at `intake_observation` (`intake_skipped: partnership_disabled`); root cause: eval UUID substring `b2b` false-triggered partnership classifier; remaining 32 `not_run`; Gmail triggers=4 replies=3 drafts=0; permanent quarantine (`orphaned_r4_attempt_9`); Attempts 1–8 unchanged; `R4_LIVE_CAMPAIGN=PENDING`; `R5_CLOSURE=PENDING`. Evidence: `storage/status/digital-coworker-r4-reconciliation-2b955a1.json`, `storage/status/digital-coworker-r4-attempt9-orphan-registry.json`.)

2026-08-07 (R4 PTB-SEM-0023 no-send intake suppression closure postmerge @ `2b955a1b9db6d38460dae881e4211cbe014fe775` — PR #180 squash-merged CI PASS; root cause: R4 `_execute_no_send` treated authoritative `newsletter_disabled` intake skip as hard failure; fix adds explicit PTB-SEM-0023→`newsletter_disabled` contract only; send path unchanged; Attempt 8 historical 20/20 send + PTB-DCQ-0088 lifecycle preserved as evidence; `r4_live_gmail_scenario_eligibility=36/36`; exact registration 20/20+16/16; PTB-DCQ-0000→0002 PASS; PTB-DCQ-0088 materialization PASS; PTB-SEM-0023 expected suppression PASS; PTB-SEM-0024 quarantine unchanged; API=worker=`2b955a1`; registration probe PASS (0 Gmail writes); Attempts 1–8 permanent quarantine; no `--execute`; no Attempt-9 approval; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_ATTEMPT_9_READY_FOR_MANUAL_APPROVAL`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` remain PENDING; automatic Gmail false; production activation false. Evidence: `storage/status/r4-nosend-intake-suppression-postmerge-2b955a1.json`, `storage/status/r4-attempt8-sem-0023-forensic-cdee2c0.json`.)

2026-08-07 (R4 live campaign attempt 8 STOPPED — executor `cdee2c07b94958208ce88a44565eaf10e97d06e5` / candidate `b7fd95e…`; campaign_id `92305703-a8ca-4e91-8b16-c13ebebb6655`; manual approval validated; send 20/20 PASS incl. PTB-DCQ-0088 full approval lifecycle; no-send 8/16 PASS; `PTB-SEM-0023` FAIL at `no_send_verification` (`intake_skipped: newsletter_disabled`); root cause: verifier lacked authoritative intake-suppression contract for newsletter no-send; remaining 7 no-send `not_run`; Gmail replies=20 triggers=20 drafts=0; LLM calls=0; permanent quarantine (`orphaned_r4_attempt_8`); Attempts 1–7 permanent quarantine unchanged; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_LIVE_CAMPAIGN=PENDING`; `R5_CLOSURE=PENDING`; automatic Gmail false; production activation false. Evidence: `storage/status/digital-coworker-r4-reconciliation-cdee2c0.json`, `storage/status/digital-coworker-r4-execute-stopped-cdee2c0-attempt8.json`, `storage/status/digital-coworker-r4-attempt8-execute-reconciliation-cdee2c0.json`.)

2026-08-07 (R4 PTB-DCQ-0088 approval materialization closure postmerge @ `cdee2c07b94958208ce88a44565eaf10e97d06e5` — PR #178 squash-merged CI PASS; root cause: `should_materialize_r4_0088_from_job` required `r4_reviewed_body_snapshot` on job input but live Gmail intake only persisted `registration_context`; fix resolves R4 0088 contract from `live_eval.registration_context` when snapshot absent; production-equivalent path verified (orchestrator → ACTION_DISPATCH → pending approval → bind lookup readiness); `r4_live_gmail_scenario_eligibility=36/36`; `exact_send_registration_payload_ready=20/20`; `exact_no_send_registration_payload_ready=16/16`; PTB-DCQ-0000→0002 regression PASS; PTB-DCQ-0088 materialization regression PASS; R3 regression PASS; API=worker=`cdee2c0`; registration probe PASS (0 Gmail writes); Attempt 7 root cause fixed (hold skipped ACTION_DISPATCH); Attempts 1–7 permanent quarantine; no `--execute`; no Attempt-8 approval artifact; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_ATTEMPT_8_READY_FOR_MANUAL_APPROVAL`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + registry remain PENDING; automatic Gmail false; production activation false. Evidence: `storage/status/r4-0088-materialization-postmerge-cdee2c0.json`, `storage/status/r4-attempt7-0088-forensic-37ca965.json`.)

2026-08-07 (R4 live campaign attempt 7 STOPPED — executor `37ca9654358d878d010431bfac207c0f6f7cd3a2` / candidate `b7fd95e…`; campaign_id `462aad8c-8266-4ee7-bc83-3831368a136b`; manual approval validated; 16/20 send PASS (`PTB-DCQ-0000`…`0080`); `PTB-DCQ-0088` FAIL at `reviewed_body_bind` (`no pending approval`); root cause: hold→pending materialization never ran because R4 contract inputs missing from job `live_eval`; remaining 19 `not_run`; Attempt-7 Gmail triggers=17 replies=16 drafts=0 external writes=0; LLM calls=0; candidate regeneration=0; permanent quarantine (`orphaned_r4_attempt_7`); Attempts 1–6 permanent quarantine unchanged; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_LIVE_CAMPAIGN=PENDING`; `R5_CLOSURE=PENDING`; automatic Gmail false; production activation false. Evidence: `storage/status/digital-coworker-r4-reconciliation-37ca965.json`, `storage/status/digital-coworker-r4-execute-stopped-37ca965-attempt7.json`, `storage/status/digital-coworker-r4-attempt7-execute-reconciliation-37ca965.json`.)

2026-08-07 (R4 live-Gmail scenario eligibility hardening postmerge @ `37ca9654358d878d010431bfac207c0f6f7cd3a2` — PR #176 squash-merged CI PASS; context-bound R4 reviewed-live eligibility evaluated before legacy 2F.2/full-system gate in `validate_live_gmail_registration`; same production safety chain wired into exact-payload validation, registration readiness, live probes, and full JIT; `r4_live_gmail_scenario_eligibility=36/36`; `exact_send_registration_payload_ready=20/20`; `exact_no_send_registration_payload_ready=16/16`; PTB-DCQ-0000→0002 production-safety regression PASS; PTB-SEM-0024 local quarantine PASS (registry eligibility only); API=worker=runner=`37ca965`; full read-only live JIT PASS; registration probe PASS (0 Gmail writes); Attempt 6 root cause fixed (generic allowlist before R4 path); Attempts 1–6 permanent quarantine; no `--execute`; no Attempt-7 approval artifact; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_ATTEMPT_7_READY_FOR_MANUAL_APPROVAL`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + registry remain PENDING; automatic Gmail false; production activation false. Evidence: `storage/status/r4-live-gmail-eligibility-postmerge-37ca965.json`, `storage/status/r4-live-gmail-eligibility-readiness-9a3db03.json`.)

2026-08-07 (R4 live campaign attempt 6 STOPPED — executor `f56f2eab30cd54b4ad828556a8d3940d4c03f89c` / candidate `b7fd95e…`; campaign_id `298aeee7-dc72-4614-86eb-8f20566bee2f`; manual approval validated; `PTB-DCQ-0000` PASS (full pipeline, `thread_match=true`, 1 trigger + 1 Gmail reply); `PTB-DCQ-0002` FAIL at `inbound_trigger_send` before second Gmail trigger (`POST /admin/live-eval/runs` → `400` — `scenario_id 'PTB-DCQ-0002' is not allowlisted for live Gmail (2F.2 or campaign)`); root cause: `validate_live_gmail_registration()` invoked generic `require_scenario_allowed_for_live_gmail()` before R4 context-bound registry path; remaining 34 `not_run`; Attempt-6 Gmail triggers=1 replies=1 drafts=0 external writes=0; LLM calls=0; candidate regeneration=0; permanent quarantine (`orphaned_r4_attempt_6`); Attempts 1–5 permanent quarantine unchanged; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_LIVE_CAMPAIGN=PENDING`; `R5_CLOSURE=PENDING`; automatic Gmail false; production activation false. Evidence: `storage/status/digital-coworker-r4-reconciliation-f56f2ea.json`, `storage/status/digital-coworker-r4-execute-stopped-f56f2ea-attempt6.json`, `storage/status/digital-coworker-r4-attempt6-orphan-registry.json`.)

2026-08-07 (R4 exact registration payload hardening postmerge @ `f56f2eab30cd54b4ad828556a8d3940d4c03f89c` — PR #174 squash-merged CI PASS; authoritative registration payload builder + exact-payload matrix wired into execute path, readiness API, live JIT, and probes; `exact_send_registration_payload_ready=20/20`; `exact_no_send_registration_payload_ready=16/16`; registration probe PASS (0 Gmail writes); structural dry-run PASS; full read-only live JIT PASS; mailbox baseline PASS; Attempt 5 forensic root cause documented (`registration_context missing` / sender allowlist on PTB-DCQ-0002); Attempts 1–5 permanent quarantine; no `--execute`; no Attempt-6 approval artifact; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_ATTEMPT_6_READY_FOR_MANUAL_APPROVAL`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + registry remain PENDING; automatic Gmail false; production activation false. Evidence: `storage/status/r4-exact-registration-postmerge-f56f2ea.json`, `storage/status/r4-attempt5-registration-forensic-08e1996.json`.)

2026-08-06 (R4 live campaign attempt 1 STOPPED — executor `1c77b2f70f1c97bd689311d356caf6087e929bce` / candidate `b7fd95e…`; campaign_id `fb36fd42-ce05-492e-8227-f1aad537868b`; manual approval validated; execute-process full JIT + mailbox baseline PASS; first scenario `PTB-DCQ-0000` fail-closed at register-run before Gmail trigger (`422` — API schema Literal rejects `ai_mode=reviewed_live_llm_body`); remaining 35 scenarios `not_run`; `resume_forbidden=true`; Gmail sends=0 drafts=0 triggers=0 external writes=0; LLM calls=0; candidate regeneration=0; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + registry remain PENDING; automatic Gmail false; production activation false. Evidence: `storage/status/digital-coworker-r4-reconciliation-1c77b2f.json`, `storage/status/digital-coworker-r4-live-campaign-attempt1-summary-1c77b2f.md`.)

2026-08-05 (R4 reviewed-live backend wiring postmerge @ `1c77b2f70f1c97bd689311d356caf6087e929bce` — PR #167 squash-merged CI PASS; dual SHA binding PASS (`candidate=b7fd95e` / `executor=1c77b2f`); API=worker=runner=`1c77b2f`; `backend_wired=true`; `execute_backend_type=r4_live_eval_semi_auto_backend`; `execute_callback_available=true`; locked candidate/manifest/review/body-hash bindings PASS; structural dry-run PASS; full read-only live JIT PASS (`live_probes_collected=true`); mailbox baseline PASS; Gmail sends=0 drafts=0 triggers=0 external writes=0; no `--execute`; no signed approval artifact; execute-process approval+JIT+baseline fail-closed; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + `PROFILE_DRIVEN_DIGITAL_COWORKER_REPLY_QUALITY_QUALIFIED` remain PENDING; automatic Gmail false; production activation false. Evidence: `storage/status/r4-postmerge-verify-1c77b2f.json`.)
2026-08-05 (R4 reviewed-live executor postmerge @ `9fd262fe068d565709991f9958de76d280637dcc` — PRs #161–#165 squash-merged CI PASS; dual SHA binding PASS (`candidate=b7fd95e` / `executor=9fd262f`); locked candidate/manifest/review/body-hash bindings PASS; structural dry-run PASS; full read-only live JIT PASS (`live_probes_collected=true`); mailbox baseline PASS; Gmail sends=0 drafts=0 triggers=0 external writes=0; no `--execute`; no signed approval artifact; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + registry remain PENDING; automatic Gmail false; production activation false. Superseded as executor tip by `1c77b2f`.)

2026-08-05 (R4 reviewed-live executor implemented on branch `feature/r4-reviewed-live-campaign-executor` — dual SHA binding, approval artifact contract, registration, trusted snapshot, PTB-DCQ-0088 R4-only hold→pending materialization, sequential fail-closed loop, full live JIT + mailbox baseline; no `--execute`; no signed approval artifact; Gmail writes=0; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PASS`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + registry remain PENDING.)

2026-08-05 (R4 human review PASS — runtime SHA `b7fd95e075c16feee93a116a6062e402c1fee3df`; scored artifact `storage/status/digital-coworker-r4-human-review-scored-b7fd95e.json` (sha256 `7dced592907fb6fc…`); 10 PASS + 10 PASS_WITH_NOTE + 0 FAIL + 0 PENDING; body-hash bindings 20/20; average dimension 4.621/5; post-review preflight + dry-run PASS without regenerating candidates; Gmail sends=0 drafts=0 triggers=0 external writes=0; `--execute` not run; JIT execute-ready true pending separate manual approval artifact; `R4_HUMAN_REVIEW=PASS`; `R3_LIVE_CANARY=PASS`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + `PROFILE_DRIVEN_DIGITAL_COWORKER_REPLY_QUALITY_QUALIFIED` remain PENDING; automatic Gmail false; production activation false.)

2026-08-05 (R4 strict constrained-LLM candidates — squash PR #158 merge SHA `b7fd95e075c16feee93a116a6062e402c1fee3df`; write-free live LLM candidate package PASS: 20/20 `constrained_llm_v1` / model `gpt-4o-mini-2024-07-18` / prompt `coworker_constrained_llm_v5`; provenance_audit_pass=true; candidate_package_semantic_hash `6e6c37aaa57df146…`; authorized human-review package 20 PENDING (no PASS markings); prior `29407be` package retained as NON_QUALIFYING_DIAGNOSTIC (`PROVENANCE_AUDIT_FAIL` / `live_llm_not_invoked`); Gmail sends=0 drafts=0 triggers=0 external writes=0; `--execute` not run; `R3_LIVE_CANARY=PASS`; `R4_HUMAN_REVIEW=PENDING`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + `PROFILE_DRIVEN_DIGITAL_COWORKER_REPLY_QUALITY_QUALIFIED` remain PENDING; automatic Gmail false; production activation false. Gate doc: `docs/reply-quality/r4-renderer-provenance-gate.md`.)

2026-08-05 (R4 scaffolding merged — squash PR #156 merge SHA `29407be98ef6feef4e3494f3f60435c3ed0de5d6`; campaign_type `coworker_r4_live_quality_campaign` / `r4_reviewed_live_candidate` / `live_llm`; locked 36 scenarios (20 send / 16 no-send, ≥15 families); postdeploy candidate-generation PASS + dry-run PASS on eval stack; Gmail sends=0 drafts=0 triggers=0 external writes=0; human review PENDING (execute readiness false); `--execute` fail-closed; `R3_LIVE_CANARY=PASS`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + `PROFILE_DRIVEN_DIGITAL_COWORKER_REPLY_QUALITY_QUALIFIED` remain PENDING; automatic Gmail false; production activation false. Note: later provenance audit classified this candidate package as NON_QUALIFYING_DIAGNOSTIC — superseded by `b7fd95e`.)

2026-08-05 (R4 scaffolding prepared on branch `feature/r4-live-coworker-quality-campaign` — campaign_type `coworker_r4_live_quality_campaign`, execution_mode `r4_reviewed_live_candidate`, ai_mode `live_llm`; locked 36-scenario registry (20 send / 16 no-send, ≥15 families); write-free candidate generation + human-review package + dry-run runner; R3 PASS prerequisite documented; R4 live `--execute` remain blocked pending separate manual confirmation; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + registry remain PENDING; automatic Gmail false; production activation false; Gmail sends/drafts/triggers in this slice = 0.)

2026-08-05 (R3 digital-coworker frozen live canary attempt 8 PASS — runtime/runner SHA `5e9b1839d9a4ac5ac6aef1795d88a2eff5f06517`; tenant `TENANT_LIVE_EVAL`; campaign `54f2f10b-4f09-4ae4-9950-39bd2efb1214`; mode `r3_frozen_approved_body`; reply provider `live_eval_recipient_env` / `google_mail`; 8/8 new approved Gmail replies + 7/7 no-send verified; drafts created 0; unknown outcomes 0; attempts 1–7 excluded from PASS counts; `PTB-DCQ-0088` hold_for_review → R3 override `approval_required` → pending → frozen bind → explicit approval → Gmail; `R3_LIVE_CANARY=PASS`; `R4_LIVE_CAMPAIGN` + `R5_CLOSURE` + `PROFILE_DRIVEN_DIGITAL_COWORKER_REPLY_QUALITY_QUALIFIED` remain PENDING; production activation not implied. Formal record: `docs/reply-quality/r3-live-canary-result.md`.)
...
| Decision contract (2B) | `Verified — ADDED 2026-07-20` | `policy_authorization` internal truth; legacy `auto_execute`/`send_for_approval` in decisioning fail-closed; dispatch-boundary action authorization; per-action approvals; dispatch skips integration-disabled external writes before approval materialization (`integration_not_allowed`); legacy job-level approvals superseded at per-action materialization; `tests/test_decision_contract.py`, `test_action_authorization.py`, `test_action_dispatch_authorization_boundary.py`, `test_superseded_approval_lifecycle.py` |
...
| Gold dataset & adversarial coverage (2E) | `Verified — MERGED` | **Merged to `main` @ `3baae50` (PR #2, 2026-07-21).** Deterministic evaluation foundation only — **not** Live Gmail or Live LLM. 20 gold/adversarial fixture scenarios; manifest `k2e-v1`; baseline `k2e-baseline-v1`; schema `2e.1`; `hash_algorithm`: `semantic-json-v2`; `manifest_hash`: `600e7fd601227d0e327951df8f2a91f48eb6af713410f2a76f819d4db5a793d8`. Verification @ merge-head: smoke 10/10 PASS; full dataset 20/20 PASS; coverage gate PASS; baseline PASS; `real_external_calls=0`; `pg_eval` 7/7 PASS (0 skips); full backend 3768 PASS (0 failures); frontend PASS; Docker PASS (Release Gate run `29860392395`). Next: Kapitel 2F (Live Gmail + Live LLM E2E testbot); 2G (AI-generated scenarios, fuzzing, volume). |
| Live evaluation foundation (2F.1) | `Verified — MERGED` | **Merged to `main` @ `122b469` (PR #6, 2026-07-22).** Authoritative `live_eval_runs` registry; atomic root-job claim; trusted Gmail intake snapshot; write-policy + telemetry; migrations 017–019; trust anchor (`POST /jobs` strips `live_eval`); session-safe idempotency. **No live Gmail transport or live LLM execution in 2F.1.** Verification @ merge-head (Release Gate `29879941376`): 2E smoke 10/10 PASS (`real_external_calls=0`); full backend 3912 PASS (0 failures); live-eval PG 5/5; migration 016 PG 1/1; telemetry idempotency PG 2/2; frontend PASS; Docker PASS. Next: Kapitel 2F.2 (Live Gmail Transport Testbot). |
| Live Gmail transport testbot (2F.2) | `Verified — CLOSED` | **Authoritative Live Gmail success:** workflow run `30050565974`, evaluation_run_id `77d87e8f-d6a3-427c-a2cc-e25c5995968a`. Merged implementation @ `4d343f8` (PR #11, 2026-07-22); live S01 verified via manual `live_gmail_transport` workflow. Gmail and Live LLM verified in separate external runs — not one combined run. |
| Live LLM E2E testbot (2F.3) | `Verified — CLOSED` | **Authoritative Live LLM success:** workflow run `30131333378`, evaluation_run_id `ed492673-bcca-4fb2-be3d-3e4653dcb709`. Report semantics fixed @ `b344701`. Historical harness failure `30125105087` has `provider_outcome=unknown`, is excluded from authoritative evidence, and must not be rerun as success. |
| Live Eval evidence, replay and closure (2F.4) | `Closure candidate — implementation complete` | Deterministic evidence manifest (`2f.4.evidence-manifest`), offline replay (`2f.4.replay-report`), merge-stable `evidence_source_descriptor_hash`, and Release Gate job `final-2f-evidence` on push to `main`. **Closure declaration:** `Kapitel 2F — PASS och stängt`. The declaration becomes authoritative only when post-merge job `final-2f-evidence` succeeds on the same `main` SHA and produces `2f_final_report.json` with `overall_status=passed` inside artifact `2f-final-evidence-<main-sha>`. No new Gmail or OpenAI runs required. Next baseline for 2G is locked to the successful 2F closure SHA. |
| Generated scenario evaluation (2G) | `Closure candidate — implementation complete` | Deterministic template generator (`2g-generator-v1`), mutation engine (`2g-mutation-v1`), PR batch (60) and main batch (160), blocking quality gates, and Release Gate jobs `2g-pr-eval`, `2g-main-eval`, `final-2g-evidence`. **Closure declaration:** `Kapitel 2G — PASS och stängt`. The declaration becomes authoritative only when post-merge job `final-2g-evidence` succeeds on the same `main` SHA and produces `2g_final_report.json` with `overall_status=passed` inside artifact `2g-final-evidence-<main-sha>`. No new Gmail or OpenAI runs required. |
