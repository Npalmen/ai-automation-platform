...
| Gold dataset & adversarial coverage (2E) | `Verified` | 2026-07-21: manifest `k2e-v1` (20 scenarios); smoke 10/10; full dataset 20/20; `k2e-baseline-v1.json`; schema `2e.1`; `semantic-json-v2` manifest hash with static generation provenance; coverage gate executable; CI smoke+regression in `release-gate.yml`; `real_external_calls=0`; S18 approval-resume telemetry stable under `ENV=test` |
