...
- [x] Integration Health Center — GET /integrations/health (per-system gmail/monday health from internal signals only: config_present + scanner_ran + inbox_sync/dispatch_success; no external API calls; no secrets in response; overall_status healthy/warning/error; recent_errors list); Integrationshälsa card in Dashboard UI; 47 tests; 1589 total pass
- [x] Pilot Readiness Hardening — GET /pilot/readiness (11 deterministic checks: auth_configured, tenant_exists, onboarding_ready, integrations_health_not_error, routing_ready_for_lead, dispatch_duplicate_protection, dispatch_observability, scheduler_safe, required_env_present, ui_available, test_lead_exists; overall_status ready/almost_ready/not_ready; score counters); Pilotberedskap card in Dashboard UI; 49 tests; 1638 total pass
- [x] Super Admin Panel v1 — GET /admin/tenants/overview (read-only; aggregates all DB tenants; per-tenant: onboarding/pilot/integration/dispatch 30d/errors/latest_activity; top-level counts + total_hours_saved_30d; no secrets; one failing tenant does not abort rest); "Super Admin" tab in UI (summary cards + tenant table + Öppna kund); 44 tests; 1682 total pass
- [x] Admin Auth Hardening — ADMIN_API_KEY env var + X-Admin-API-Key header; require_admin_api_key FastAPI dependency (constant-time comparison; 401 on missing/wrong/unconfigured; secret never in response); GET /admin/tenants/overview now requires admin key (tenant X-API-Key rejected); UI: Admin API-nyckel input in Super Admin tab (localStorage-persisted); 24 tests; 1706 total pass
- [x] Support Layer v1 — TenantSupportContext + 5 support modules (analyzer/missing_info/question_generator/prioritizer/response_draft/next_action); SUPPORT_ANALYSIS step in CUSTOMER_INQUIRY pipeline (try/except wrapped — pipeline-safe); PATCH /jobs/{job_id}/support-status + POST /jobs/{job_id}/support-regenerate + GET /dashboard/support; 13 support fields in GET /cases/{job_id}; Support-analys panel + KPI section in UI; 95 tests; no LLM; no external writes
- [x] Admin Customer Provisioning — POST /admin/tenants (201, api_key shown once), GET /admin/tenants (no raw keys), POST /admin/tenants/{id}/rotate-key (new key shown once), PATCH /admin/tenants/{id}/status (active/inactive); TenantApiKeyRecord model + TenantApiKeyRepository (SHA-256 hashed keys, kw_ prefix); TenantConfigRecord extended with slug/status/created_at/updated_at; auth refactored DB-first → env fallback → dev-mode; inactive tenant 403 at auth layer; schema_migrations extended; UI: create-tenant form, show-once key banner, tenant table, rotate/activate/deactivate actions; 39 tests; 1995 total pass
- [x] Approval-Based Email Dispatch — auto_actions gate for send_customer_auto_reply/send_internal_handoff/send_email; _email_needs_approval(); _EMAIL_ACTION_TYPES constant; _create_email_approval_record() creates approval_requests row with next_on_approve="email_send" + delivery_payload; Monday always executes; approve_request/reject_request route email approvals to _resolve_email_approval(); execute_action called on approve, suppressed on reject; send errors captured without crash; UI: email approval cards with recipient/subject/body + Godkänn och skicka/Avvisa; 37 tests; 2032 total pass
