...
- [x] Kapitel 2F.1 — Live evaluation foundation — **merged to `main` @ `122b469` (PR #6, 2026-07-22).** Registry, atomic claim, trusted intake, write-policy, telemetry, migrations 017–019, CI PG gates (5/5 + 1/1 + 2/2); docs `10f-live-eval-testbot.md`; no live Gmail send or live LLM execution.
- [x] Kapitel 2F.2 — Live Gmail Transport Testbot (implementation) — **merged to `main` @ `4d343f8` (PR #11, 2026-07-22).** Two-process testbot + app delivery/intake; hermetic contracts; security review F-01–F-07 closed; manual `live_gmail_transport` workflow built (`workflow_dispatch` only). **No real Gmail send or live LLM execution yet.** Verification @ merge-head (Release Gate `29926558386`): 2E smoke 10/10 PASS (`real_external_calls=0`); full backend 4002 PASS; live-eval PG 5/5; migration 016 PG 1/1; telemetry idempotency PG 2/2; hermetic 2F.2 tests 107 PASS (local); frontend PASS; Docker PASS.
- [ ] **Kapitel 2F.2 — Operator configuration and first Live Gmail S01 verification** (dedicated Gmail test accounts, GitHub `live-gmail-eval` environment secrets, label/filter setup, protected workflow dispatch, one S01 send, delivery + exact-message intake, zero app replies, zero live LLM, exact cleanup, report review). **Not started.**
- [ ] **Kapitel 2F.3+ — Live LLM E2E** (optional live LLM execution; runtime provenance in harness reports). **Not started.**
