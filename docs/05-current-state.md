...
| Dispatch Observability + ROI Attribution | ✅ IMPLEMENTED | `app/workflows/dispatchers/observability.py` — `get_dispatch_summary(db, tenant_id, *, job_type, system, limit_recent)` reads persisted `controlled_dispatch` IntegrationEvents; aggregates total/success/failed/skipped, by_mode (full_auto/manual/approval_required/unknown), by_job_type, by_system; ROI: 5 min saved per success → estimated_minutes_saved/hours_saved; `dispatch_mode` metadata stored in every event payload (no schema change); `GET /dispatch/summary` endpoint; Dispatchöversikt card in Dashboard UI (Swedish labels); 28 tests |
...
| 1455 tests passing | ✅ | `python -m pytest` |
