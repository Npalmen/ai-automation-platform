...
| Workflow Scan Status | ✅ IMPLEMENTED | `GET /workflow-scan/status` — tenant-scoped; reads `settings.workflow_scan`; defaults: `{last_scan_at: null, systems_scanned: [], status: "never_run", summary: {}}`; scan state populated by future scanner (not yet automated) |
...
| 1130 tests passing | ✅ | `python -m pytest` |
