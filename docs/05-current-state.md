...
| Workflow Scanner Engine | ✅ IMPLEMENTED | `WorkflowScannerEngine` + `BaseWorkflowScannerAdapter` in `app/workflows/scanners/`; `ADAPTER_REGISTRY` maps system keys to adapter instances; future adapters (monday, microsoft_mail, visma, fortnox, crm) added by registering one class; engine handles persistence and no-clobber logic so adapters stay pure |
...
| 1187 tests passing | ✅ | `python -m pytest` |
