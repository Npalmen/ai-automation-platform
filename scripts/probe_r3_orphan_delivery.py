#!/usr/bin/env python3
"""Read-only orphan delivery probe for R3 live canary (no Gmail send, no run mutation)."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="R3 orphan delivery read-only probe")
    parser.add_argument(
        "--evaluation-run-id",
        default="05839824-1fb1-4e98-b8e1-b8025df5db3d",
        help="Evaluation run ID for orphan attempt 3",
    )
    parser.add_argument(
        "--tenant-id",
    ...
    parser.add_argument(
        "--classification",
        default="orphaned_attempt_3_delivery_probe_verified",
